...
/obj/map_metadata/stalag
	ID = MAP_STALAG
	title = "Stalag XIII (125x125x2)"
	...
	var/modded_num_of_SSTV2 = FALSE
	var/modded_num_of_prisoners2 = FALSE
	faction_distribution_coeffs = list(GERMAN = 0.3, USA = 0.70)
	songs = list(
		"MGS4 - Midnight Shadow:1" = 'sound/music/shadow.ogg'
		)
/obj/map_metadata/stalag/germans_can_cross_blocks()
	return (processes.ticker.playtime_elapsed >= 600 || admin_ended_all_grace_periods)

/obj/map_metadata/stalag/soviets_can_cross_blocks()
	return (processes.ticker.playtime_elapsed >= 1800 || admin_ended_all_grace_periods)


/obj/map_metadata/stalag/job_enabled_specialcheck(var/datum/job/J)
	. = TRUE
...
//		J.total_positions = max(5, round(clients.len*0.75))
		...
		if (!J.is_prisoner)
			...
			if (istype(J, /datum/job/usa/uk_soldier_prisoner))
				J.total_positions = max(10, round(clients.len*0.45*3))
			if (istype(J, /datum/job/usa/squad_leader_prisoner) && !modded_num_of_prisoners2)
				J.total_positions = max(1, round(clients.len*0.1*3))
				modded_num_of_prisoners2 = TRUE
	return .

/obj/map_metadata/stalag/announce_mission_start(var/preparation_time)
	world << "<font size=4>Americans have <b>3 minutes</b> to prepare before the ceasefire ends! Germans can cross after <b>1 minute</b>. <br>The Germans will win if they hold out for 50 minutes without any escapes. The Americans will win if a prisoner manages to escape.</font><br><br><br><b>PLEASE READ THE WIKI FOR RULES! http://mechahitler.co.nf/wiki/media/index.php?title=Camp_guide</b><br>"

/obj/map_metadata/stalag/reinforcements_ready()
	return (germans_can_cross_blocks() && soviets_can_cross_blocks())

/obj/map_metadata/stalag/short_win_time(faction)
	return 1200

/obj/map_metadata/stalag/long_win_time(faction)
	return 1200

var/no_loop3 = FALSE

/obj/map_metadata/stalag/update_win_condition()
	...
	if (world.time >= 30000)
		...
		return FALSE
	if ((current_winner && current_loser && world.time > next_win) && no_loop3 == FALSE)
		ticker.finished = TRUE
		var/message = "A prisoner has escaped! The American prisoners have won the round!"
		world << "<font size = 4><span class = 'notice'>[message]</span></font>"
		show_global_battle_report(null)
		win_condition_spam_check = TRUE
		no_loop3 = TRUE
